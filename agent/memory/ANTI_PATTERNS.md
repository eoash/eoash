...
[2026-03-09] data-source.ts의 grace period(`d.date > graceCutoff`) 절대 `>=`로 바꾸지 말 것. Prometheus 카운터 첫 등장일에 `increase([1d])`가 전체 누적값을 외삽 (ash 3/8: 실제 622K → 외삽 6.9M, 11배 스파이크). 데이터 누락 의심 시 grace period를 완화하지 말고, `generate_backfill.py`로 로컬 transcript에서 backfill 보충할 것.
