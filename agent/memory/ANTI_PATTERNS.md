...
[2026-03-10] **Prometheus `sum by` 집계 + 누적 카운터 = 스파이크 함정**. `sum by (user_email, model)`로 누적 카운터를 집계하면, stale 시리즈 탈락/재등장 시 급변이 발생. JS delta 계산으로 보정 시도했으나, 양의 delta 자체가 비정상적으로 클 수 있음. `cache_read_tokens`이 특히 취약 (chiri 21.7B 스파이크). **반드시 Prometheus raw counter를 직접 확인한 후** delta 로직 수정할 것. 추측으로 수정하면 반복 실패.
