...
[2026-03-10] **Prometheus `sum by` + 카운터 리셋 = 이중 집계**. OTel Collector 재시작 시 stale 시리즈 탈락(음의 delta) 후 새 시리즈가 0부터 재누적. 양의 delta만 합산하면 리셋 전 + 리셋 후 이중 집계됨 (실사례: Chiri output 80M, 실제 ~1.5M). **해결**: `computeDailyIncrease()`에 reset recovery 모드 — 음의 delta 시 peak 기록, 카운터가 peak 초과할 때까지 양의 delta 무시. **주의**: 신규 설치자의 첫 데이터가 recovery에 걸릴 수 있음 → 점검 필요.
