...
| `_SYNC_Withtax` | **수동 관리** (위드택스) | type, year, item, 1월~12월 |

### _SYNC_Withtax 탭 구조 (수동 관리)
위드택스 데이터는 외부 세무법인 Looker Studio 기반 → Apps Script 자동 동기화 불가 → **월 1회 수동 업데이트**

| type | year | item | v1(1월) | ... | v12(12월) |
|------|------|------|---------|-----|----------|
| METADATA | | lastUpdated | 2026.01.21 | | |
| MONTHLY | 2025 | 서비스매출 | 71556494 | ... | |
| MONTHLY | 2025 | 당기순이익 | -114242550 | ... | |
| YEARLY | 2022 | 매출합계 | 2026752125 | | |
| EXPENSE | 2025 | 급여 | 1185947260 | 28.4(비율) | |

**초기 탭 생성**: Apps Script → `📊 Dashboard Sync` → `📋 Withtax 탭 초기 생성 (1회)` 실행

**월별 업데이트 방법** (위드택스 결산 후):
1. Looker Studio 열기: `https://lookerstudio.google.com/s/mrHnGOvqVDQ`
2. `_SYNC_Withtax` 탭 → MONTHLY 행에서 해당 월 값 수정
3. YEARLY 2026 행 추가 (연간 결산 후)
4. METADATA의 lastUpdated 날짜 변경

**대시보드 반영**: 5분 이내 자동 갱신 (revalidate=300)
