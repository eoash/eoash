...
출력 폴더명을 `output/YYYYMMDD_HHMMSS_{topic_slug}` 형식으로 생성한 뒤 렌더링합니다.
- `YYYYMMDD_HHMMSS`: 실행 시각 (예: `20260225_143022`)
- `{topic_slug}`: 주제를 영문/숫자/언더스코어로 축약 (예: `AI_에이전트팀`, `건강한식습관`)
- 기존 `output/` 폴더는 건드리지 않음

```bash
OUTPUT_DIR="output/$(date +%Y%m%d_%H%M%S)_{topic_slug}"
...
렌더링 완료 후 `output/{YYYYMMDD_HHMMSS_{topic_slug}}/` 디렉토리에 `slide-01.png` ~ `slide-0N.png` 파일이 생성됩니다.
...
- **글씨 크기 일관성** ⚠️: 모든 슬라이드의 headline/body/subtext 폰트 크기가 슬라이드 간 균일한지 반드시 확인. 들쭉날쭉하면 수정 필요.
  - headline: 슬라이드 타입이 같으면 크기 동일해야 함
  - body: 줄 수가 많은 슬라이드일수록 폰트가 작아지지 않는지 체크
  - 텍스트 양이 많을 경우 카피를 줄여서 크기를 유지할 것 (폰트 축소 금지)
