...
- **Windows cp949 인코딩**: Python `open()` 기본 인코딩이 cp949 → UTF-8 transcript 파싱 시 `UnicodeDecodeError`. 반드시 `encoding="utf-8"` 명시
- **Windows 경로 구분자**: `"/subagents/" in path` 같은 Unix 슬래시 필터는 Windows(`\`)에서 실패 → `os.path` 함수 사용
