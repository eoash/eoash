...
- **Weekly Champions**: 주간 토큰 사용량 1위 명예의 전당
- **일별 사용량 차트**: Input / Output / Cache 스택 영역 차트
- **모델별 파이 차트**: Opus / Sonnet / Haiku 비율

### Leaderboard (`/leaderboard`)
- **AI 도구 탭**: Claude Code / Codex / Gemini 전환 (실데이터)
...
### Members (`/members`)
- 팀원 선택 드롭다운 (마지막 선택 localStorage 기억)
...
### Setup Guide (`/setup`)
- curl 원라이너 복사 버튼
- 4단계 설치 설명 + Prerequisites + Troubleshooting

### 공통 기능
- **한/영 전환**: Sidebar 하단 `한국어/EN` 토글 (localStorage 저장)
- **공지 배너**: 설정 파일 기반 AnnouncementBanner (setup/update/info 타입)
- **모바일 반응형**: 햄버거 드로어 + 터치 tooltip 지원
...
| Gemini Tokens | 네이티브 OTel (Gemini CLI 내장) | input + output + cache + thought |

### 데이터 소스 (유저별 자동 분기)

각 유저의 backfill JSON에서 가장 마지막 날짜(max_date)가 자동으로 cutoff 기준점이 됩니다.
Prometheus는 cutoff + 1일부터 데이터를 사용합니다 (OTel `increase([1d])` 외삽 보정).

| 구간 | 데이터 소스 | 설명 |
|------|-----------|------|
| cutoff 이전 | **Backfill JSON** (`src/lib/backfill/*.json`) | install-hook.sh 실행 시 transcript 파싱으로 생성 |
| cutoff ~ cutoff+1일 | **건너뜀 (grace period)** | OTel delta→cumulative 외삽 보정 |
| cutoff+1일 이후 | **Prometheus 실시간** | 세션 종료 시 hook이 자동 전송 |
...
| 단계 | 내용 |
|------|------|
| [1/7] | `otel_push.py` 다운로드 |
| [2/7] | Claude Code Stop hook 등록 (세션 종료 시 자동 수집) |
| [3/7] | 과거 transcript backfill → API 전송 |
| [4/7] | Codex CLI 세션 데이터 1회 수집 |
| [5/7] | Codex 자동 수집 cron 등록 (2시간마다) |
| [6/7] | Gemini CLI OTel 설정 (`~/.gemini/settings.json`) |
| [7/7] | Gemini CLI 정상 전송 확인 |
...
| Gemini CLI | 세션 종료 시 (내장 OTel) | Gemini 네이티브 telemetry → OTel Collector 직접 전송 |
...
│   ├── members/page.tsx          # Members (개인별 상세)
...
│       ├── codex-usage/route.ts  # GET /api/codex-usage
│       └── gemini-usage/route.ts # GET /api/gemini-usage
├── components/
│   ├── layout/Sidebar.tsx        # 좌측 네비게이션
│   ├── cards/                    # KpiCard, TeamRankingCard
│   ├── charts/                   # Recharts 기반 차트
│   └── leaderboard/
│       └── LeaderboardTable.tsx  # 리더보드 (Claude + Codex + Gemini 탭)
└── lib/
...
### 웹에서 쓰는 AI (claude.ai, chatgpt.com 등)는 왜 추적이 안 되나요?

이 대시보드는 **터미널 기반 AI 도구** (Claude Code, Codex CLI, Gemini CLI)만 추적합니다.

| 구분 | 터미널 도구 (추적 가능) | 웹 서비스 (추적 불가) |
|------|------------------------|---------------------|
| 동작 환경 | 로컬 머신 (우리 인프라) | 서비스 회사 서버 |
| 데이터 소유 | 우리가 로그 접근 가능 | Anthropic/OpenAI/Google이 보유 |
| 계측 방법 | OTel hook, 세션 로그 파싱 | 외부에서 접근 불가 |

**한 줄 요약**: 터미널 도구는 우리 로컬에서 실행되니까 데이터를 뽑을 수 있고, 웹은 각 회사 서버 안에서 끝나버려서 우리가 볼 수가 없습니다.

> 💡 각 서비스의 Usage 페이지에서 본인 사용량을 직접 확인하는 것은 가능합니다.
